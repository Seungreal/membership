...
public class PhoneApp{
    public static void main(String[] args) {
        ...
        while(true) {
            System.out.println("0.Exit 1.집전화걸기 2.전화받기 3.휴대폰걸기 4.아이폰서치 5.삼성페이결제");
            switch (scanner.next()) {
            case "1":
                phone=new Phone();
                phone.setKind("집접화");
...
class IPhone extends CelPhone{
    ...
    @Override
    public String toString() {
        return String.format("%s %s을 사용해서 %s중에 %s 을 한다",
                KIND, company, move, search);
    }
}
class GalPhone extends IPhone{
    public final static String KIND = "갤럭시폰";
    ...
    @Override
    public String toString() {
        return String.format("%s %s 제품을 사용해서, %s중에  %s 한다",
